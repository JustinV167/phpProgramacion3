...
class SentencesStruct
{
   ...
   public $allCategoryQuery = "SELECT *,(SELECT i.rute as img_rute FROM img_rute as i where i.id=c.id_img), (SELECT COUNT(*) FROM products  as p WHERE  p.id_category=c.id AND p.status!='inactive' )as n_products  FROM categorys as c WHERE c.status!='inactive'; ";
   public $categoryQuery = "SELECT *,(SELECT i.rute as img_rute FROM img_rute as i where i.id=c.id_img), (SELECT COUNT(*) FROM products  as p WHERE  p.id_category=c.id AND p.status!='inactive')as n_products  FROM categorys as c WHERE c.status!='inactive' AND c.name ILIKE '%' || ? || '%';";
   ...
   public $productsQuery = "SELECT *,
      (SELECT i.rute FROM img_rute as i WHERE p.id_img=i.id) as rute_img
      FROM products as p WHERE p.status!='inactive' AND p.id_category=? AND p.name ILIKE '%' || ? || '%';";
